...
public:
    ...
    ll recur(int ind, int inv){
        if(dp[ind][inv]!=-1)return dp[ind][inv];
        if(inv>((ind+1)*(ind)/2))return 0;
        if(ind==0){
            if(inv==0)return 1;
            return 0;
        }
        ...
        int ans=0;
        for(int i=inv;i>=max(0,inv-ind);i--){
            ans+=recur(ind-1,i);
            ans%=mod;
        }
    ...
    int numberOfPermutations(int n, vector<vector<int>>& requirements) {
      ...
      for(int i=0;i<requirements.size();i++){
          for(int j=0;j<=400;j++){
              if(j==requirements[i][1])continue;
              dp[requirements[i][0]][j]=0;
          }
      }
      int reqd=0;
      for(int j=0;j<=400;j++){
